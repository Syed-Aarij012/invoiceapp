...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (stored) {
    ...
    fetch('data.json')
      .then(r => r.json())
      .then(data => { invoices = data; save(); renderInvoices(); });
  }
...
function save() { localStorage.setItem('invoices', JSON.stringify(invoices)); }
...
function showDetail(id) {
  ...
  const sa = inv.senderAddress || {};
  const ca = inv.clientAddress || {};
...
function markCurrentPaid() {
  ...
  if (inv) { inv.status = 'paid'; save(); showDetail(currentInvoiceId); renderInvoices(); }
}

function markPaid(id) {
  const inv = invoices.find(i => i.id === id);
  if (inv) { inv.status = 'paid'; save(); renderInvoices(); }
}

// Delete
function deleteCurrentInvoice() { openDeleteModal(currentInvoiceId); }
function deleteInvoice(id) { openDeleteModal(id); }
...
function confirmDelete() {
  ...
  save(); closeDeleteModal(); showList(); renderInvoices();
}
...
// Save Invoice
function saveInvoice(statusArg) {
  ...
  if (editMode) {
    ...
    const newStatus = statusArg === 'keep'
      ...
      : statusArg;
    Object.assign(inv, data, { status: newStatus });
    save(); closeForm(); showDetail(currentInvoiceId); renderInvoices();
  } else {
    invoices.push({ id: generateId(), status: statusArg === 'keep' ? 'pending' : statusArg, ...data });
    save(); closeForm(); renderInvoices();
  }
}
